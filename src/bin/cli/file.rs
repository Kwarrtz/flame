...
impl FlameSource {
    ...
    pub fn from_file(path: impl AsRef<Path>) -> Result<FlameSource, CliError> {
        let contents = std::fs::read_to_string(path.as_ref())?;
        Ok(match path.as_ref().extension().ok_or(CliError::ExtensionError)?.to_str() {
            Some("json") => FlameSource::from_json(&contents)?,
            Some("ron") => FlameSource::from_ron(&contents)?,
            _ => return Err(CliError::ExtensionError)
        })
    }
}
...
impl TryFrom<FlameSource> for Flame {
    ...
    fn try_from(src: FlameSource) -> Result<Flame, CliError> {
