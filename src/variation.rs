    ...
    pub fn eval(
        ...
        let (xo, yo) = match self {
            ...
            BrokenHandkerchief => {
                let r = (x * x + y * y).sqrt();
                let theta = x.atan2(y);
                ((theta + r).sin(), (theta - r).cos())
            }
            Heart => {
                let r = (x * x + y * y).sqrt();
                let theta = x.atan2(y);
                let (s, c) = (theta * r).sin_cos();
                (r * s, -r * c)
            }
            Disc => {
                let r = (x * x + y * y).sqrt();
                let theta = x.atan2(y);
                let t = theta * FRAC_1_PI;
            ...
            Fisheye => {
                let r = (x * x + y * y).sqrt();
                (2.0 * y / (r + 1.0), 2.0 * x / (r + 1.0))
            }
            Eyefish => {
                let r = (x * x + y * y).sqrt();
                (2.0 * x / (r + 1.0), 2.0 * y / (r + 1.0))
            }
            Exponential => {
                let e = (x - 1.0).exp();
            ...
            Julia => {
                ...
                let a = r.sqrt();
                let sgn = rv.lambda();
                let (s, c) = (theta / 2.0 + rv.omega()).sin_cos();
                (a * sgn * c, a * sgn * s)
            }
            Blur => {
                let a = rv.psi();
            ...
            Fan => {
                ...
                let (s, c) = (theta + sgn * t / 2.0).sin_cos();
                (r * c, r * s)
            }
            // parametric
            Blob(w, h, l) => {
            ...
            Fan2(a, b) => {
                ...
                let (s, c) = (theta + sgn * p1 / 2.).sin_cos();
                (r * s, r * c)
            }
            Rings2(val) => {
                let r = (x * x + y * y).sqrt();
            ...
            RadialBlur(angle) => {
                ...
                let (sin_p1, cos_p1) = p1.sin_cos();
                let t2 = phi + t1 * sin_p1;
                let t3 = t1 * cos_p1 - 1.0;
                let (s, c) = t2.sin_cos();
                (r * c + t3 * x, r * s + t3 * y)
            }
